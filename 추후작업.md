...
## 4. 감리사례 expander — `clean_text` 미적용 (pages.py:194)

### 배경
`pages.py:194`의 감리사례 렌더링이 `clean_text` + `unsafe_allow_html=True` 없이 렌더됨:
```python
adjusted = raw_content.replace("# ", "### ").replace("## ", "#### ")
st.markdown(adjusted)  # ← clean_text 미적용, unsafe_allow_html 미사용
```

### 영향
감리사례 내부의 문단 참조가 파란색 볼드로 강조되지 않음.
현재 감리사례 데이터에 문단 참조가 드물어 체감 영향은 낮음.
...
1. 잘깎은 사례집 만들기 -> 시연용 + test용

2. conclusion guide까지 다 만들었는데 추론모델이 필요한가?
- pydantic ai 로 강제해서 만들면되는거아닌가?

3. 어떻게 처리해야하는지 물어보는사람 vs 계산법을 물어보는사람