...
def title_for(metadata: dict) -> str:
    ...
    Why: 제목(회계기준원이 단 실무 요약)은 일상어 질문과의 매칭을 끌어올리는 1급 신호.
    QNA는 metadata.title, 없으면 hierarchy 말단으로 폴백.
    """
...
def load_pdr_to_atlas():
    ...
    for qna in raw_qnas:
        ...
        parent_docs_to_insert.append(
            {"_id": qna_id, "content": full_text, "metadata": metadata}
        )
    ...
    print("1. 기존 Parent 컬렉션 데이터를 초기화합니다...")
