...
```
**지식그래프 기반 검색 시스템 구축** (자세한 설명 : [4. 기술 설명 ② RAG — 지식그래프](#4-기술-설명-②-rag--지식그래프))
...
결론 재현에 실패한 7건의 사례는 유형별로 분류하여 [미재현 7건](#미재현-7건)에 기술<br>
...
**질문**(QNA-2017-I-KAQ015-Q1) : 아파트 분양계약으로 고객이 1차 중도금 납부 전엔 해제 가능(위약금 분양대금의 10%), 이후엔 해제불가. 이때 회사가 수행완료분에 대한 집행 가능한 지급청구권을 가져 진행기준을 적용할 수 있는가?
  ...
  3. **의사결정 트리 활용**
  - 1에서 에서 진입된 개념이 순차대로 트리에 주입
    - ① 고객의 인수 ← `고객의 인수` (1단계)
    - ② **기간에 걸쳐 vs 한 시점 인식** ← `대체 용도가 있는 자산을 만들지 않는다(35⑶)` (3단계)
    - ③ 변동대가(추정) ← `변동대가` (3단계)
    - ④ 변동대가 제약 ← `변동대가 추정치를 제약함` (3단계)
    - ⑤ 계약의 식별(5요건) ← `계약을 식별함` (4단계)
...
실제 구현 : [`app/nodes/analyze.py`](app/nodes/analyze.py) · [`app/domain/graph.py`](app/domain/graph.py)

```mermaid
graph TD
    Q["사용자 질의 + 최근 3턴"] --> A["analyze — gpt-4.1-mini 1회"]
    A --> G{"routing<br/>1115호 IN / OUT"}
    G -->|OUT| STOP["범위 밖 거절 · 파이프라인 종료"]
    G -->|IN| SG{"scope guard<br/>1115호 앵커 존재?"}
    SG -->|없음| STOP
    SG -->|있음| E1["1순위 · 주제 지목"]
    E1 --> E2["2순위 · 위계 확장"]
    E2 --> E3["3순위 · 용어사전"]
    E3 --> E4["4순위 · 코사인"]
    E4 --> M["concept_ids 확정<br/>순서대로 병합 · 중복 제거"]
    M --> R["retrieve 노드"]
```
...
| 단계      | 개념 탐색 방법                                         | 세부사항                                                                        |
| --------- | ---------------------------------------------------------- | --------------------------------------------------------------------------- |
| **1단계** | LLM이 질의의 핵심 토픽 지목                            | 핵심 개념 80개중 선택 |
...
실제 구현 : [`app/domain/graph.py`](app/domain/graph.py) — [`resolve_question`](app/domain/graph.py#L222) · [`traverse`](app/domain/graph.py#L268) / 그래프 빌드 : [`app/preprocessing/14~20-ontology-*.py`](app/preprocessing) / 데이터 : [`data/ontology/`](data/ontology)
...
- 질의회신·감리지적 연결 방법
  ...
  - 문단 인용이 미흡한 사례(123건) : 사례의 제목을 활용하여 연결성을 보강
...
| 상호참조       | 264개 (타 기준서로 연결된 외부 참조 3개 포함)                            |
| 부록B 대응     | 3개 (본문 규정과 부록B 연계)                                             |
| 5단계 선행판단 | 7개 (수익인식 5단계에 따른 선행-후행 흐름)                               |
| 사례           | 총 188개 (질의회신 101건, 감리사례 22건,적용사례(IE) 65건)               |
| 사례→문단 인용 | Q663개 (질의회신 600개 + 감리지적 63개)                                  |
| 사례→개념 연결 | 74개 (질의회신 9개 + 적용사례 65개)                                      |
| 예외 처리      | 8건 (질의회신 6건,적용사례 2건) → 인용 조문이 없어 개념 노드에 직접 매핑 |
| 용어사전       | 용어 423개 → 개념과 간선 606개                                           |
| BC(결론근거)   | 총 1,021개 (BC-문단 간선 446개, BC-개념 간선 516개, 사례-BC 간선 59개)   |
...
| 사례 → 문단·개념 | 인용            |     737 | 사례가 판단 근거로 삼은 규정 (문단 663건 · 개념 74건) |
...
실제 구현 : [`match_judgment_tree`](app/domain/graph.py#L154) (app/domain/graph.py) — 진입 개념 순서대로 트리를 이어붙여 [`app/nodes/generate.py`](app/nodes/generate.py#L99)에 주입 / 데이터 : [`data/ontology/judgment_trees.json`](data/ontology/judgment_trees.json) (41개)
...
실제 구현 : [`app/agents.py`](app/agents.py) — [`ClarifyOutput`](app/agents.py#L135) · [`clarify_agent`](app/agents.py#L248) · [`output_validator`](app/agents.py#L301)
...
| ------------------- | -------------------------------- | ------------------------------------------------------------------- |
| 질의 재구성         | LLM 질의 재작성에 전면 의존      | LLM 질의 재작성은 용어 사전 매핑에만 사용, 나머지 3단계는 원문 유지 |
...
| 4   | **LLM 확률변동 잔존**                | 초기 질의 분석과 최종 답변 생성에 LLM의 추론이 개입 → 생성형 AI를 활용한 챗봇의 한계             |     |
| 5   | **GraphRAG 방법론중 일부만 채택**    | 노드는 기준서 문서 (목차 소제목·문단·사례)이고 간선은 포함·인용 → 엔티티·스키마를 추출 단계 없음 |     |
