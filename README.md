...
> **지금 바로 써보기 → http://134.185.104.224:8501/** (Oracle Cloud 배포, Streamlit)
...
```
  qna-parents 92건 (질의자·회계기준원·해석위원회 작성 원문)
        │
        │
        ├─ 복수 질문 분리 ──▶ 질의1·2가 뭉친 5건을 질문별로 나눔
        │
...
| 버킷 (건수)       | 왜 탈락했나                                                                                                             | 고칠 수 있나?                                                                                                                           |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| **진입 누락 (2)** | 질문이 정답 개념을 용어로도, 토픽으로도 제시를 하지 않아 진입점 진입 실패.                                              | **진입점 강화로 개선 여지** — 코사인 진입은 실익 1건 + 비결정성으로 폐기했으나, 결정적 진입점 강화(용어사전 별칭 보강 등)로는 회수 가능 |
| **복합규정 (4)**  | 재매입↔풋옵션↔반품권처럼 규정이 서로 얽힌 복합거래라 답변이 정답과 다른(그러나 연결된) 문단을 인용                      | **수정 필요 없음** — 진짜 오답이 아니라 인용 문단 매칭 차이                                                                             |
| **1115호 밖 (6)** | 정답이 1115호 범위 밖 — 라우팅이 아예 거절(2)하거나, 정답이 다른 기준서(재고 1002·충당 1037·무형자산 등) 문단을 요구(4) | **수정 필요 없음** — 추후 기준서 확장으로 해결 가능                                                                                     |
| **헤지 (2)**      | Case 분류(A/B 분기)까지 맞혔으나 — 확정 결론으로 좁히지 않고 양쪽을 병기. Case분기를 집착해 확정을 유보                 | **생성 개선 여지** — 결론 확정을 유도하면 가능. 단, 애매한 경우 정답을 확정하는 것 보단 Case분기가 과도한게 낫다는 판단                 |
...
이미 회계 기준서는 명시적인 상호 참조 구조(5단계 모형·문단 간 참조·본문과 부록B의 연결)를 가지는데, 단순 유사도와 근거 없는 가중치를 사용한 것이 문제였음
이를 해결하기 위해 기존의 확률 기반 검색 진입부를 개편하고, 비슷한 것을 추측하는 대신 연결된 것을 추출하는 지식 그래프로 전환
AI와 사람의 개입을 최소화

| 임베딩(유사도)              | 온톨로지(지식그래프)                 |
| --------------------------- | ------------------------------------ |
| 비슷한 것 위주 유사도 상위 N개 추출   | "연결된 것" 명시적 관계              |
| 확률적 · 설명 불가          | 결정적 · 근거를 경로로 제시          |
| 오답이 내용 오염으로 나타남 | 오류가 "검색 실패"로 안전하게 드러남 |
