...
설계상 배제한 범위가 아니라, 현재 이 프로젝트가 **입증 못 했거나 감당 못 한** 지점만.

| #   | 한계                              | 성격                                                                                                                                                                                      |
| --- | --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **확장성 — 수작업 큐레이션 반복** | 용어사전·판단트리·개념 위계가 1115호 하나에 수작업 큐레이션됨 → 기준서 추가마다 같은 노동 반복 · 개정 시 유지보수 방안 미비. "AI 개입 최소화"가 장점이자 비용                             |
| 2   | **단위 테스트·CI 전무**           | `app/test`는 전부 홀드아웃 평가 스크립트(`run.py`·`grade.py`·`exp_*`) — pytest 회귀 스위트·자동 검증 CI 없음. 외부 API(임베딩·리랭커·LLM) 의존이라 결정론 CI엔 오프라인 fixture 선행 필요 |
| 3   | **베이스라인 대조군 없음**        | 전제인 "범용 LLM은 환각·비재현"을 논리·설계로만 제시 → 같은 92건을 범용 LLM에 기준서 원문 넣고 돌린 대조군 수치는 미측정                                                                  |
| 4   | **LLM 비결정성 완전 제거 실패**   | 결정론 골격(점수·클러스터)은 불변, 최종 generate만 LLM이라 잔존. 미재현 `202511019`·`202504C`는 정답 문단 정확 회수에도 조항 오독 · 동일 입력에도 카드 수·표현 변동 가능                  |
