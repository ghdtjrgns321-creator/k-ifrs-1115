...
```
  ...
  ① 런타임 시 질의회신 문서 유입 원천 차단   EXCLUDE_QNA=1  (graph_fetch.py:134)
        ...
        └─▶ 결과: 92건 검색결과 중 질의회신 문서 유입 0/92  ✓ (격리됨)
  ② 온톨로지 인덱스 구축 시 홀드아웃 셋 포함 문제   (미격리 — ⚠️사후 발견으로 수정)
        평가용으로 배제했어야 할 홀드아웃 92건이 용어 사전(aliases) 및 케이스 링크(case_links) 생성에 반영되는 오류발생
        → 정답지의 메타데이터가 검색 진입 경로로 누출될 위험 발생 (문서 제목이 용어로 등재된 84건, 자체 누출 18건)
        └─▶ 사후 처리 : 홀드아웃 셋을 배제한 '정제된 인덱스'를 재빌드→재생성→재채점
                 이진 기준으로 대조 (exp_contamination.py)
```
...
- **확정 회피 / 헤지 (4)** — `KQA009-Q1` · `201803A` · `201803B` · `36964`
  ...
  - 사실관계를 대입해 하나로 확정하지 않고 Case A/B를 병기(예: 201803B는 문단 35·37·38을 다 인용해놓고 기간/한시점을 병기만 함)
  - **원인: 설계된 "유보적 접근"(2종 오류 회피 — 애매하면 확정 대신 조문·분기를 제시)의 부작용.** 
...
- **라우팅 거부 (2)**
  ...
  - `38673`(환불부채) — 질문이 표면적으로 *"제1109호(금융부채)냐 제1037호(충당부채)냐"* 만 물어 **1115호를 한 글자도 언급하지 않는다** → LLM이 "타 기준서 질문"으로 오판·거부.
  - 실제 라우팅 실패 사례로 **정답 기준서가 질문 표면에 드러나지 않는 함정형**
  - 2종 오류(틀린 확답) 회피용 보수적 라우팅이 "질문에 *언급된* 기준서"를 기준으로 판단하는 한 구조적으로 취약하다는 사실이 드러남
