...
- **진입 누락 (2)** — *진입점 강화로 개선 여지*
  질문이 정답 개념을 용어로도 토픽으로도 제시하지 않아 진입 실패. 코사인 진입은 실익 1건 + 비결정성으로 폐기했으나, 결정적 진입점 강화(용어사전 별칭 보강 등)로는 회수 가능.
- **복합규정 (4)** — *수정 필요 없음*
  재매입↔풋옵션↔반품권처럼 규정이 서로 얽힌 복합거래라, 답변이 정답과 다른(그러나 연결된) 문단을 인용. 진짜 오답이 아니라 인용 문단 매칭 차이.
- **1115호 밖 (6)** — *수정 필요 없음(추후 기준서 확장으로 해결)*
  정답이 1115호 범위 밖 — 라우팅이 아예 거절(2)하거나, 정답이 다른 기준서(재고 1002·충당 1037·무형자산 등) 문단을 요구(4).
- **헤지 (2)** — *생성 개선 여지*
  Case 분류(A/B 분기)까지 맞혔으나 확정 결론으로 좁히지 않고 양쪽을 병기(Case 분기에 집착해 확정 유보). 결론 확정을 유도하면 개선 가능하나, 애매한 경우엔 확정보다 Case 분기가 낫다는 판단도 있음.
