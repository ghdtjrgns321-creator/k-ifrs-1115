...
대부분의 RAG가 임베딩 유사도로 "비슷한 문서"를 끌어오지만, 이 시스템은 기준서가 이미 가진 **명시적 관계(개념·문단·사례·용어를 잇는 지식그래프)** 위에서 근거를 결정적으로 탐색한다.

검색 진입부에 유사도·가중치·리랭커가 없다 — "비슷한 것" 대신 **"연결된 것"** 을 가져온다.
...
개요의 파이프라인을 노드 단위로 뜯어본다.

LangGraph 없이 순수 Python으로 오케스트레이션하고, 각 노드는 부분 결과를 상태에 병합한 뒤 SSE로 진행 상황을 스트리밍한다.
