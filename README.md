...
- 핵심 설계
  - 지식그래프 : 기준서의 목차 위계와 문단 간 상호참조 관계를 노드와 간선으로 구조화(개념 80개, 문단 250개, 사례 188개)
...
2. 사전 확장
   - 진입 어휘 확정
     - 초안 288개 + 사례 제목 132개(질의회신과 감리지적사례의 제목) + 부록A 공식 정의 9개
   - 개념에 연결
     - 각 단어를 그래프 개념에 연결
...
| ---------------- | --------------- | ------: | ------------------------------------------------------ |
| 개념 ↔ 개념      | 계층            |      79 | 상위와 하위 주제 간 종속 관계 (부모-자식)              |
...
<p align="center"><sub>전체 간선을 켠 모습. E3 상호참조(노란선)와 E2 선행판단(빨간선)이 개념과 문단을 관계로 잇는다.</sub></p>
...
- **유보적 판단으로 인한 확정 실패 (2건)** — `202503008`(계약 해지 위약금), `38648`(성과보상 인센티브)
  ...
  - **원인: "유보적 접근"(애매하면 확정 대신 조문과 분기를 제시)의 설계 부작용.**
...
- **라우팅 거부 (2)**
  - `KQA006-Q2`(우선매수협상권과 통제 이전) — 판매후리스(1116호)가 중심 내용이라 범위 밖 판정 - 라우팅 성공
...
| 문서 DB         | MongoDB Atlas                   | 기준서와 사례 원문 저장/조회                          |
