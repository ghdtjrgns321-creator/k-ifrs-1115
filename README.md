...
- 수록 문서 ([전체 목록](#데이터-구성))
  - 기준서 원문 : 규정 247문단, 결론도출근거(BC) 652문단, 적용사례(IE) 65건
...
실제 한국회계기준원 질의회신 90건으로 구성된 테스트 셋을 활용하여 성능을 검증

| 평가 지표                | 1회차             | 2회차             | 3회차             |
| ------------------------ | ----------------- | ----------------- | ----------------- |
| **결론 재현**            | **95.6%** (86/90) | **93.3%** (84/90) | **93.3%** (84/90) |
| 정답 문단 회수           | 92.1% (151/164)   | 90.9% (149/164)   | 92.1% (151/164)   |
| 질의회신 문서 유입(순환) | 0% (0/90)         | 0% (0/90)         | 0% (0/90)         |
| 실행 에러                | 0% (0/90)         | 0% (0/90)         | 0% (0/90)         |

3회 모두 실패한 3건과 회차마다 갈린 5건의 분석은 [결론 미재현 원인 분석](#결론-미재현-원인-분석)에 기술<br>
...
| **3단계** | 코사인 유사도 상위 3개 지목                 | K=3,5,7 비교 결과 유의미한 차이가 없어 최소값 채택 |
...
| 순회 | 사례 → 개념  | 쟁점            |       244 | 사례가 다투는 논점 (질의회신·감리 179, 적용사례 65)           |
| 순회 | 사례 → 개념  | 직결            |         9 | 인용 조문이 없는 사례를 개념에 직접 매핑                      |
| 순회 | BC → 문단    | 논의            |     4,861 | 목차 표기로 이은 BC→본문 (BC 510/652, 문단 230)               |
| 순회 | BC → 문단    | 언급            |       327 | BC 본문이 "문단 N"을 인용 (BC 190개)                          |
| 순회 | BC → 개념    | 배경            |       516 | 제정 취지                                                     |
| 경계 | 용어 → 개념  | 진입            |       606 | 질의에서 개념으로 진입 (입구)                                 |
...
2. **결론도출근거(BC) : 결론 참고용 근거로만 사용**
   - 방대한 분량(본문의 5배) → 질의 1건당 평균 232문단이 검색되는 비효율
...
4. **질의회신·감리지적사례 : '인용', '쟁점' 간선 분리**
   - **인용 (1,220개)** : 사례 본문이 근거로 제시한 문단
     ...
     - 답변 생성에 포함할 경우, 기본개념 조문에 사례가 과도하게 집중되는 문제(개념 1개당 사례 37건)
...
3. 신뢰성 확보 (2종 프롬프트 3회씩 총 6회 반복 수행)
  ...
  - 만장일치 자동 채택 : 결과가 만장일치인 경우에만 자동 채택 (207문단)
  - 수동 교차 검증 : 의견이 갈린 33개 문단 → 원문 직접 읽고 수동 판정 및 근거 기재
...
| ------------- | -------------------------- | ----------------- | ----------------- | ----------------- |
| 라우팅        | 1115호 대상 질문 여부 판정 | 98.9% (89/90)     | 98.9% (89/90)     | 98.9% (89/90)     |
| 문단 수집     | 정답 문단을 수집           | 92.1% (151/164)   | 90.9% (149/164)   | 92.1% (151/164)   |
| 결론 재현     | 정답과 같은 방향           | **95.6%** (86/90) | **93.3%** (84/90) | **93.3%** (84/90) |
| 질의회신 유입 | 정답 문서를 자기 인용      | 0% (0/90)         | 0% (0/90)         | 0% (0/90)         |
| 실행 에러     | 90건 전수 완주             | 0% (0/90)         | 0% (0/90)         | 0% (0/90)         |
...
| 케이스                          | 판정(1·2·3회) | 발생 원인 (변동 상세)                                                                                                                        |
| ------------------------------- | ------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `202003D` (교육훈련원가)        | 0 · 1 · 0     | 정답 : "IAS 38 적용범위라 1115호 문단 95가 **적용되지 않음**" / 1·3회차는 문단 95 안에서 자산화 가능으로 답변                                |
| `36904` (용역 진행기준)         | 1 · 0 · 1     | 정답 : "**문단 35의 통제 이전 조건을 충족할 때만** 기간에 걸쳐 인식" / 2회차는 아 조건을 빼고 진행률 측정(문단 39)만으로 서술                |
| `38697` (반품금액 추정 불가)    | 1 · 0 · 0     | 정답 : "통제 이전 시점에 **재고자산은 제거**하고 수익만 유보" / 2·3회차는 재고자산 제거까지 유보 대상으로 묶음                               |
| `202511019` (수취채권·계약자산) | 1 · 0 · 1     | 정답 : "시간 경과만으로 무조건적 권리가 성립하므로 **수취채권**"(문단 105·108) / 2회차는 계약자산으로 결론                                   |
| `201909A` (연착·취소 보상)      | 1 · 1 · 0     | 정답 : "**변동대가**(문단 50~59) 적용, 문단 B33의 손해배상이 아님" / 3회차는 확정을 피하고 두 견해를 병치, 제시한 기준대로면 충당부채로 귀결 |
...
| 2   | **범용 LLM과 통제된 수치 비교 불가**             | 검증셋 90건이 공시된 자료이므로 범용 LLM에 이미 노출 가능성                                                                                                               |
| 2-1 | **검증셋을 설정 선택에도 사용**                  | 1115호 관련 회신문 88건을 전부 테스트셋에 써서 따로 남겨둘 데이터가 없었고, 검색 설정·채점 기준도 이 세트를 보고 골랐다 → 처음 보는 질문의 성능은 측정치보다 낮을 수 있음 |
| 2-2 | **채점자 검증 미완**                             | 채점 기준의 자기 일관성 95%는 확인했으나 사람 라벨과 대조하지 않았다 → 일관되게 틀릴 가능성 미배제. 같은 답변 파일이 채점 회차에 따라 83~86으로 갈림                      |
