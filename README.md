...
<table>
  ...
  <tbody>
    ...
    <tr><td><b>PydanticAI 구조화 출력</b></td><td>'근거'와 '결론' 분리를 코드 수준에서 강제 — AI 답변의 변동성 억제</td></tr>
    <tr><td><b>핀포인트 + Reranker 검색</b></td><td>사전 배정 문서 직접 조회 + 실무 약칭 자동 확장("묶음 판매" → 복수의 수행의무·거래가격 배분)</td></tr>
    <tr><td><b>도메인 데이터 적재</b></td><td>기준서 원문·적용사례·질의응답·감리지적사항·교육자료 등 <b>1,575건</b>을 DB에 구축</td></tr>
    <tr><td><b>근거 선행, AI 후행</b></td><td>적재 문서를 AI 호출 <b>전에</b> SUMMARY와 함께 열람 — 근거 확인 후 답변</td></tr>
    <tr><td><b>Split View 근거 추적</b></td><td>AI 답변과 인용 근거를 한 화면에서 동시 확인 — 참조 문단은 볼드 처리</td></tr>
  </tbody>
</table>
...
범용 LLM으로 회계 기준서를 해석하면 근거 없는 답변, 곧 **환각**이 나오기 쉽다.

이는 AI를 과대신뢰하는 2종 오류로 이어진다. 이 시스템은 유연성을 포기하고 **무결성**에 집중한다.
...
데이터 적재부터 최종 답변까지 전 과정을 결정론적으로 통제한다.
...
> 통과율 88.7%의 Issue 6건은 "토픽 매칭이 기대와 다름(답변은 정확)",<br>"응답 시간이 긴 편" 수준의 **메타데이터·완성도 이슈**다.<br>잘못된 정보를 만들거나 근거 없이 단정한 사례는 **0건**이다.
