...
| 축   | GraphRAG                                                         | 이 프로젝트                                                                         |
| ---- | ---------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| 노드 | 원문에서 LLM이 뽑아낸 등장 대상(엔티티)                          | 기준서 문서 구조물 — 목차 소제목 80개 · 문단 · 사례                                 |
| 간선 | 관계의 성질을 명시 — "제약한다" · "제외한다"                     | 구조 관계 — 포함(계층) · 인용(상호참조) · 대응(부록B). 의미 관계는 5단계 순서 7개뿐 |
| 추론 | 노드 덩어리 자동 묶기 + 덩어리별 사전 요약 · 같은 대상 중복 병합 | 없음. 하위 개념군 확장 + 인용 1홉                                                   |

엔티티 추출을 안 한 것은 누락이 아니라 선택이다. LLM이 뽑은 엔티티·스키마는 검증 불가능한 층이 되고, 그것이 라우팅 근간에 들어가면 틀렸을 때 "검색 실패"가 아니라 **"그럴듯한 오답"**으로 나타난다. 기준서는 목차와 상호참조가 저자가 명시한 관계라 추출 대신 이관으로 끝나고, 그래서 위 census로 검증까지 된다. 대신 치르는 비용은 한계점 5·6번이다.
...
- QNA에는 회계기준원·해석위원회가 확정한 **정답 결론**이 이미 존재
...
| 4   | **LLM 비결정성 완전 제거 실패**                          | 결정론 골격(점수·클러스터)은 불변, 최종 generate만 LLM이라 잔존. 미재현 `202511019`·`202504C`는 정답 문단 정확 회수에도 결론이 틀어졌음, 동일 입력에도 카드 수·표현 변동 가능                                                                                                                                                                                                                                                                                                                                  |
| 5   | **완전한 GraphRAG 아님**                                 | 노드가 기준서 목차 소제목·문단·사례 문서이고, 간선은 "무엇이 무엇 아래 있다"(포함)와 "어느 문단이 어느 문단을 언급한다"(인용)뿐이다. 원문에서 등장 대상을 뽑아 노드로 만드는 단계도, 표기가 다른 같은 대상을 하나로 합치는 단계도, 개념 덩어리를 자동으로 묶어 미리 요약해두는 단계도 없다. 이 프로젝트가 한 것은 **검색 진입 방식을 유사도에서 그래프 순회로 바꾼 것 한 가지**다                                                                                                                              |
| 6   | **관련 개념을 모아주지만 무엇이 중요한지는 못 골라준다** | 목차에는 "[제약]은 [변동대가] 아래 항목"까지만 적혀 있고 "[제약]이 변동대가 금액의 **상한을 깎는다**"는 적혀 있지 않다. 그래서 그래프는 관련 개념을 다 모아주되 **그중 무엇이 결론을 뒤집는 개념인지는 표시하지 못한다** — 8개를 모아오면 8개가 같은 무게로 LLM에 넘어간다. 무엇이 중요한지 고르는 일은 판단트리와 generate LLM이 대신한다. 더 나쁜 경우는 연결선이 아예 없어 정답 개념까지 가지도 못하는 것으로, 미재현 `KQA007`(대리로 받은 금액은 거래가격에서 빼야 하는데 목차에 그 연결이 없음)이 그 예다 |
