...
```
**지식그래프 기반 검색 시스템 구축** (자세한 설명 : [3-2. RAG — 지식그래프](#3-2-기술-설명--rag--지식그래프))
...
3. 기술 설명
   1. [진입점](#3-1-기술-설명--진입점--다단계-개념-진입) — 4단계 병합으로 개념 확정
   2. [RAG — 지식그래프](#3-2-기술-설명--rag--지식그래프) — 노드·간선 구성과 구조 검증
   3. [의사결정 트리](#3-3-기술-설명--의사결정-트리) — 기준서 분기를 트리로
   4. [PydanticAI](#3-4-기술-설명--pydanticai) — 구조화 출력과 환각 차단
4. [검증](#4-검증) — 홀드아웃 92건 · 미재현 7건
5. [핵심 의사결정](#5-핵심-의사결정--거리-기반-rag에서-지식그래프로의-전환) — 검색 진입부를 왜 갈아엎었나
6. [한계](#6-한계) — 확인된 결함과 후속 과제
...
```mermaid
graph LR
    Q["질의 + 최근 3턴"] --> A["analyze<br/>gpt-4.1-mini 1회"]
    A --> G{"1115호 범위?<br/>routing · scope guard"}
    G -->|아니오| STOP["거절 · 종료"]
    G -->|예| E["개념 진입 4단계<br/>주제 지목 → 위계 확장<br/>→ 용어사전 → 코사인"]
    E --> M["concept_ids 확정<br/>순서대로 병합 · 중복 제거"]
    M --> R["retrieve"]
```
